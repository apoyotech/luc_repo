...
## - compare meta received to database and use largest(eventually switch to a request to fetch missing db meta for item)
			...
			if not self.tmdb_key: self.tmdb_key = 'a07324c669cac4d96789197134ce272b'
...
#			control.notification(title=item_title, message='Removed %s duplicate sources from list' % (len(self.sources) - len(filter)))
	...
	def getConstants(self):
		...
		if getSetting('furk.username'): self.prem_providers += [('furk', int(getSetting('furk.priority')))]
	...
	def rd_cache_chk_list(self, torrent_List, hashList):
		# Normalize hashes (hex lowercase; convert base32 to hex if needed)
		def norm(h):
			h = h.strip().lower()
		...
		try:
			...
			n_hashes = [norm(h) for h in hashList]
			def chunks(lst, n):
				for i in range(0, len(lst), n):
					yield lst[i:i+n]
			cached = {}
			for part in chunks(n_hashes, 10):
				try:
					out = RealDebrid().check_cache(part)
					if out:
						cached.update(out)
				except Exception:
					pass
			# Tag using RD response
			any_set = False
			for i in torrent_List:
				h = norm(i.get('hash',''))
				ok = bool(cached.get(h, {}).get('rd')) if isinstance(cached.get(h, {}), dict) else False
				if ok:
					i.update({'source': 'cached (pack) torrent' if 'package' in i else 'cached torrent'})
					any_set = True
				else:
					i.update({'source': 'uncached (pack) torrent' if 'package' in i else 'uncached torrent'})
			if any_set:
				return torrent_List
		except Exception:
			pass
		# Fallback to DMM bulk cache
		try:
			...
			def _process(_chunk, _id):
				try:
					cached.update(dmm.check_cache(_chunk, _id))
				except Exception:
					pass
			threads = []
			imdb_id = None
			try:
				imdb_id = self.meta['imdb']
			except Exception:
				imdb_id = self.meta.get('imdb') if isinstance(self.meta, dict) else None
			chunks = (n_hashes[i:i + chunk_size] for i in range(0, len(n_hashes), chunk_size))
			for chunk in chunks:
				th = Thread(target=_process, args=(chunk, imdb_id))
				threads.append(th)
				th.start()
			for th in threads:
				th.join()
