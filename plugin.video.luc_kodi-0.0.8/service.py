	...
	def run(self):
		...
		try:
			...
			if not repo_xml.status_code == 200:
				return control.log('[ plugin.video.luc_kodi ]  Could not connect to remote repo XML: status code = %s' % repo_xml.status_code, LOGINFO)
			repo_version = re.findall(r'<addon id=\"plugin.video.luc_kodi\".+version=\"(\d*.\d*.\d*)\"', repo_xml.text)[0]
...
#	repoVersion = control.addon('repository.luc_kodi').getAddonInfo('version')
...
#	log_utils.log('##   script.module.jacksparrowscrapers Version: %s' % str(fsVersion), level=LOGINFO)
	log_utils.log('######   chainsgenocide SERVICE ENTERING KEEP ALIVE   #####', level=LOGINFO)
except:
